...
def center_windows(root, dir, buckets):
    ...
    ) // 4  # Equal spacing on sides and between windows
...
def main():
    ...
    ttk.Label(frm_controls, text="Valor:").grid(
        row=1, column=0, padx=2, pady=4, sticky="e"
    )
            ...
            if tem == None:
                ...
                try:
                    ...
                    messagebox.showerror(
                        "Erro DIR", f"Erro ao atualizar diretório: {str(e)}"
                    )
                ...
                messagebox.showerror("Erro", "Entrada inválida: Chave duplicada")
    ...
    ttk.Button(frm_controls, text="Inserir", command=inserir_valor).grid(
        row=2, column=0, columnspan=4, pady=(6, 4), sticky="we"
    )
    ...
    ttk.Button(frm_controls, text="Buscar", command=buscar_valor).grid(
        row=2, column=0, columnspan=4, pady=(6, 4), sticky="we"
    )

    # ---------------- OUTROS ----------------

    ttk.Button(root, text="Sair", command=root.destroy).pack(
